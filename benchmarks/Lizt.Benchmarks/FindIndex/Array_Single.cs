...
namespace Lizt.Benchmarks.FindIndex
{
    ...
    public class Array_Single
    {
        ...
        public void Setup()
        {
            _source = new Single[N];
            var rand = new Random(765);
            for (int ii = 0 ; ii < _source.Length ; ii++)
            {
                _source[ii] = (Single)rand.Next(0, Int32.MaxValue - 1);
            }
            _source[N - 1] = Single.MaxValue;
        }
        ...
        public int Lizt_FindIndex()
        {
            ...
            {
                result = _source.FindIndex(Single.MaxValue);
            }
        ...
        public int SpanHelpers_IndexOf()
        {
            ...
            for (uint ii = 0 ; ii < _iterations ; ii++)
            {
                result = Array.IndexOf<Single>(_source, Single.MaxValue);
            }
