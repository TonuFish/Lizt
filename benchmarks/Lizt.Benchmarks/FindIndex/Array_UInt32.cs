...
namespace Lizt.Benchmarks.FindIndex
{
    ...
    public class Array_UInt32
    {
        ...
        public void Setup()
        {
            _source = new UInt32[N];
            var rand = new Random(765);
            for (int ii = 0 ; ii < _source.Length ; ii++)
            {
                _source[ii] = (UInt32)rand.Next(0, Int32.MaxValue - 1);
            }
            _source[N - 1] = UInt32.MaxValue;
        }
        ...
        public int Lizt_FindIndex()
        {
            ...
            {
                result = _source.FindIndex(UInt32.MaxValue);
            }
        ...
        public int SpanHelpers_IndexOf()
        {
            ...
            for (uint ii = 0 ; ii < _iterations ; ii++)
            {
                result = Array.IndexOf<UInt32>(_source, UInt32.MaxValue);
            }
